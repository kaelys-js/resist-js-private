...
└── products/
    └── storylyne/
        ├── editor/         (@/products/storylyne/editor)    — SvelteKit + shadcn-svelte editor UI
        ├── runtime/        (@/products/storylyne/runtime)   — Babylon.js HD-2D game runtime
        └── plugin-api/     (@/products/storylyne/plugin-api) — Plugin SDK for third-party extensions
```

**Import scopes:** `@/` for all packages. Shared: `@/schemas/*`, `@/utils/*`, `@/locale/*`, `@/ui/*`, `@/config/*`. Products: `@/products/[product]/[service]/*`.
...
// 3. Product packages
import { SceneManager } from '@/products/storylyne/runtime';
...
- **Plugin API is a public surface** — `@/products/storylyne/plugin-api` exports must be stable and fully documented; treat changes as breaking.
