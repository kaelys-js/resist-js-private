...
- For each hit, open the file at the reported location, read the function signature, and identify the actual type the parameter / return *should* carry (look at how the parameter is consumed inside the body, or what the surrounding type contract requires).
