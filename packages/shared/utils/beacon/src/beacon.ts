 ...
 * Client Error Beacon
 *
 * Sends PII-stripped error payloads via `navigator.sendBeacon()`.
...
const DEFAULT_BEACON_URL = '/api/errors' as const;
 ...
 * @param endpoint - The beacon endpoint URL. Defaults to `'/api/errors'`.
...
export function beaconError(
  ...
  endpoint: string = DEFAULT_BEACON_URL,
): Result<Void> {
