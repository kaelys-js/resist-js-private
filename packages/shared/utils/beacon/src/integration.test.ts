...
describe('error reporting pipeline', () => {
  ...
  it('toBeaconPayload output is deep-frozen and still validates against schema', () => {
    const captured: CapturedError = makeCaptured();
    const payloadResult: Result<BeaconPayload> = toBeaconPayload(captured);
    expect(payloadResult.ok).toBe(true);
    if (!payloadResult.ok) {
    ...
    // Frozen payload should still round-trip through JSON and validate
    const json: Str = JSON.stringify(payloadResult.data) as Str;
    const deserialized: unknown = JSON.parse(json);
    const serverResult: Result<BeaconPayload> = safeParse(BeaconPayloadSchema, deserialized);
    expect(serverResult.ok).toBe(true);
  ...
  it('rejects payload with injected PII fields after serialization', () => {
    const captured: CapturedError = makeCaptured();
    const payloadResult: Result<BeaconPayload> = toBeaconPayload(captured);
    expect(payloadResult.ok).toBe(true);
    if (!payloadResult.ok) {
    ...
    const json: Str = JSON.stringify(payloadResult.data) as Str;
    const tampered: Record<Str, unknown> = JSON.parse(json) as Record<Str, unknown>;
    tampered.user = { id: 'user-123', email: 'john@example.com' };

    // Server-side validation should reject the tampered payload (strictObject)
