...
This is a monorepo meaning the repo holds multiple packages. You can checkout the code and build all sub-projects with:
...
1. Make sure your code passes all checks (`{{ pm.run }} check`).
1. Make sure your tests pass (`{{ pm.run }} test`).
...
Run `{{ pm.run }} test` to run the tests for all sub packages.
...
To run a single integration test, provide the `FILTER` env var with the test name. E.g. `FILTER="includes paths" {{ pm.run }} test:integration`. You can also open up the file and change `test` to `test.only`.

You can run the test server with `{{ pm.run }} go` to hit it with your browser.
...
Our [Checks](https://github.com/resist-js/resist/blob/master/docs/en-US/GITHUB_CHECKS.md) will catch most styling issues that may exist in your code. You can check the status of your code styling by simply running `{{ pm.run }} check`.
...
- Ensure `{{ pm.run }} check` pass.

### Generating changelogs

For changes to be reflected in package changelogs, run `{{ pm.dlx }} changeset` and follow the prompts. All changesets should be `patch` until resist.js 1.0.
