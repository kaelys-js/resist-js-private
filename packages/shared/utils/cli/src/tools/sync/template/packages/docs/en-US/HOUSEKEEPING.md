...
From time to time the `@resist-js/core` team will review the Conformances that are in place for this project. Conformances are those rules/defaults provided by `@resistjs/conformances` and checked/applied through various methods such as when running `{{ pm.run }} check` and as part of the CI/CD lifecycle.
