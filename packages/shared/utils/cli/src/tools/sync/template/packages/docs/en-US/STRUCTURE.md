...
> - This file will be referenced for example when running `{{ pm.run }} check`, as well as: Pull Requests, Commit/Push, GitHub Actions CI.
...
> - This file will be referenced for example when running `{{ pm.run }} check`, as well as: Pull Requests, Push, GitHub Actions CI.
...
>
> - This file will be referenced when running `{{ pm.run }} changeset:version`. Which will automatically bump all versions in all packages across the mono-repo that have changes. Further details on this process, it's purpose can be found in `GITHUB_SETUP.md` in the `base` of the project.
