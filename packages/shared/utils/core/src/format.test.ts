...
describe('formatDuration', () => {
  ...
  it('returns seconds with 2 decimals for values < 60000', () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.data).toBe('1.50s');
    }
  });

  it('returns minutes and seconds for values >= 60000', () => {
    const result: Result<Str> = formatDuration(90_000 as NonNegativeNumber);
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.data).toBe('1m 30.0s');
    }
  });
