<script module lang="ts">
  ...
  export const LanguageSwitcherPropsSchema = v.strictObject({
    ...
    switchLanguage: v.custom<(code: Str) => void>(
...
<DropdownMenu.Sub onOpenChange={handleSubOpenChange} {...restProps}>
  ...
  <DropdownMenu.SubContent
    class="max-h-80 flex flex-col overflow-hidden bg-white/[0.08] dark:bg-white/[0.06] backdrop-blur-2xl backdrop-saturate-150 border-white/[0.12] shadow-[0_8px_32px_rgba(0,0,0,0.4),0_0_1px_rgba(255,255,255,0.1)_inset]"
  >
    ...
    <div class="shrink-0 px-2 pt-1.5 pb-2 border-b border-border/50">
      <div class="relative">
        ...
        <input
          ...
          class="h-8 w-full rounded-md bg-background/50 pl-8 {searchQuery
            ? 'pr-8'
            : 'pr-3'} text-xs outline-none border border-border/50 focus:border-ring transition-colors"
          value={searchQuery}
          oninput={(e: Event) => {
    ...
    <div class="flex min-h-0 flex-1 flex-col overflow-y-auto" use:lockHeight>
      {#each filteredLanguages as lang (lang.code)}
        <DropdownMenu.Item
          onclick={() => validated.switchLanguage(lang.code)}
          aria-current={validated.locale === lang.code ? 'true' : undefined}
          textValue={lang.endonym}
        >
