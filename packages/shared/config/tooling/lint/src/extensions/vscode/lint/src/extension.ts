...
async function showToolInstallPicker(tools: ToolDefinition[], cwd: string): Promise<void> {
  ...
  if (selected && selected.length > 0) {
    ...
    const otherTools = selected.filter((s) => !['npm', 'pip', 'brew'].includes(s.tool.installType));
    ...
    if (npmTools.length > 0) {
      const packages = npmTools
        .map((t) => t.tool.installCommand.replace('pnpm add -D ', ''))
        .join(' ');
    ...
    if (pipTools.length > 0) {
      const packages = pipTools
        .map((t) => t.tool.installCommand.replace('pip install ', ''))
        .join(' ');
    ...
    if (brewTools.length > 0) {
      const packages = brewTools
        .map((t) => t.tool.installCommand.replace('brew install ', ''))
        .join(' ');
