...
function getParamName(param: AstNode): string {
	if (param.type === 'Identifier') return (param.name as string) ?? '<unknown>';
	if (param.type === 'AssignmentPattern') {
		const left = param.left as AstNode | undefined;
		if (left?.type === 'Identifier') return (left.name as string) ?? '<unknown>';
	}
	if (param.type === 'RestElement') {
		const arg = param.argument as AstNode | undefined;
		if (arg?.type === 'Identifier') return (arg.name as string) ?? '<unknown>';
	}
	return '<destructured>';
}
...
const rule: TypeScriptRule = {
	...
	visitor: {
		ExportNamedDeclaration(node: AstNode, context: VisitorContext): LintResult[] {
			if (isExempt(context.file)) return [];
			...
			for (const param of params) {
				...
				if (!typeNode || typeNode.type !== 'TSUnionType') continue;
				...
				const isExemptUnion: boolean = EXEMPT_UNION_PATTERNS.some((p: RegExp): boolean => p.test(unionText));
				if (isExemptUnion) continue;
