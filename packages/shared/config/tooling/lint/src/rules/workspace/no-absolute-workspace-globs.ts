/**
 * Rule: workspace/no-absolute-workspace-globs
 *
 * Ensures workspace globs in pnpm-workspace.yaml use relative paths,
...
const rule: WorkspaceRule = {
  id: 'workspace/no-absolute-workspace-globs',
  description: 'Workspace globs must use relative paths, not absolute paths.',
  scope: 'workspace',
      ...
      if (glob.startsWith('/')) {
        results.push(
          createResult(
            ...
            1,
            'error',
            `Absolute path not allowed in workspace glob: ${glob}`,
            {
              tip: "Use relative globs like 'packages/*' instead",
            },
          ),
        );
      }
