/**
 * Rule: workspace/no-trailing-slash-globs
 *
 * Ensures workspace globs in pnpm-workspace.yaml do not end
...
const rule: WorkspaceRule = {
  id: 'workspace/no-trailing-slash-globs',
  description: 'Workspace globs must not end with a trailing slash.',
  scope: 'workspace',
      ...
      if (glob.endsWith('/')) {
        results.push(
          createResult(
            ...
            1,
            'error',
            `Invalid workspace glob with trailing slash: ${glob}`,
            {
              tip: 'Remove trailing slashes from globs',
            },
          ),
        );
      }
