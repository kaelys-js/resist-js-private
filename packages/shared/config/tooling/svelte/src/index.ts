...
function resolveTemplatePaths(): TemplatePaths {
  ...
  if (!appHtmlResult.ok) throw appHtmlResult.error; // integration boundary: module init
  const errorHtmlResult: Result<Path> = resolveTemplatePath('templates/error.html');
  if (!errorHtmlResult.ok) throw errorHtmlResult.error; // integration boundary: module init
...
function buildAliasesFromTsconfig(root: Path): Result<Record<Str, Str>> {
  ...
  if (!tsconfigPathResult.ok) return tsconfigPathResult;
  ...
  for (const [alias, targets] of pathEntries) {
    ...
    if (!resolvedPathResult.ok) return resolvedPathResult;
...
export function createSvelteConfig(options: CreateSvelteConfigInput): Config {
  ...
  if (!aliasesResult.ok) throw aliasesResult.error; // integration boundary: SvelteKit doesn't understand Result
  const aliases: Record<Str, Str> = aliasesResult.data;

  const gitResult: Result<Str> = getGitCommitShort();
  if (!gitResult.ok) throw gitResult.error; // integration boundary: SvelteKit doesn't understand Result
