...
describe('FeatureFlagsSchema', () => {
	it('accepts empty object with all 26 defaults true', () => {
		...
		expect(flags.headerUserNotifications).toBe(true);
		expect(flags.headerUserShortcuts).toBe(true);
		expect(flags.headerUserSettings).toBe(true);
	...
	it('has exactly 26 flag keys', () => {
		const result = safeParse(FeatureFlagsSchema, {});
		expect(result.ok).toBe(true);
		if (!result.ok) return;
		expect(Object.keys(result.data)).toHaveLength(26);
	});

	it('accepts partial override with only some flags', () => {
		const result = safeParse(FeatureFlagsSchema, {
			breadcrumb: false,
			sidebarToggle: false,
		...
		expect(result.data.headerUserDropdown).toBe(true);
	});
	...
	it('accepts partial override with existing flags', () => {
		...
		expect(result.data.showCharts).toBe(false);
		expect(result.data.themeSelection).toBe(true);
	});

	it('all flag keys are present in schema entries', () => {
		const expectedFlags = [
			'showCharts',
			'showInflation',
			'showProjections',
