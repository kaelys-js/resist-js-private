 ...
 * ```
 * [Finances] LCP 2450ms ⚠ needsImprovement   (amber)
 * [Finances] CLS 0.05 ✓ good                  (green)
 * [Finances] INP 650ms ✗ poor                  (red, console.warn)
 * ```
 *
 ...
 *
 * Uses the application name prefix (e.g. `[Finances]`) in cyan, the metric
 ...
 * // Console: %c[Finances] %cLCP %c2450ms %c⚠ needsImprovement
 * // Styled:  cyan prefix, white name, amber value, amber rating
 *
 * @example
 * logVital('CLS', 0.05, 'good');
 * // Console (dev only): %c[Finances] %cCLS %c0.05 %c✓ good
