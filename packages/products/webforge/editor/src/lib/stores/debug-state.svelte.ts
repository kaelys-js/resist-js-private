 ...
 * Persists to localStorage under the app-prefixed key (e.g. `'storylyne:debug-state'`).
 *
 * @module
 */
 ...
 *
 * @param url - Optional URL to parse `sl.*` debug params from
 * @returns `Result<DebugStore>` — always ok
 *
 * @example
 * ```typescript
 * const result = createDebugStore(new URL('http://localhost?sl.debug=true'));
