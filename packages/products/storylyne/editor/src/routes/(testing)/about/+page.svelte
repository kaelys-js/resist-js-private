<script lang="ts">
  ...
  const filteredSections: AboutSection[] = $derived.by((): AboutSection[] => {
    ...
    if (sortMode === 'name-asc') {
      result = [...result].toSorted(
        (a: AboutSection, b: AboutSection): Num => a.title.localeCompare(b.title) as Num,
      );
    } else if (sortMode === 'name-desc') {
      result = [...result].toSorted(
        (a: AboutSection, b: AboutSection): Num => b.title.localeCompare(a.title) as Num,
      );
    }
  ...
  const isCustomized: Bool = $derived(
    (viewMode !== 'sections' || sortMode !== 'default' || activeCategories.length > 0) as Bool,
  );
        ...
        <DropdownMenu.Content align="end" sideOffset={4}>
          ...
          >
            ...
            <DropdownMenu.SubContent class="w-56">
              ...
              {@const sortOpts = [
                { v: 'default', l: 'Default', d: 'Natural order' },
