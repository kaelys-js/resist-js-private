  ...
  const filteredSections: SupportSection[] = $derived.by((): SupportSection[] => {
    ...
    if (sortMode === 'name-asc') {
      result = [...result].toSorted(
        (a: SupportSection, b: SupportSection): Num => a.title.localeCompare(b.title) as Num,
      );
    } else if (sortMode === 'name-desc') {
      result = [...result].toSorted(
        (a: SupportSection, b: SupportSection): Num => b.title.localeCompare(a.title) as Num,
      );
    }
        ...
        <DropdownMenu.Content align="end" sideOffset={4}>
          ...
          >
            ...
            <DropdownMenu.SubContent class="w-56">
              ...
              {@const sortOpts = [
                { v: 'default', l: 'Default', d: 'As defined' },
