...
<div class="flex flex-1 min-h-0 flex-col overflow-hidden" data-testid="dev-toolbar-app-state">
  ...
  >
    <h3 class="text-sm font-semibold text-foreground inline-flex items-center gap-2">
      <Settings2 class="size-4 text-primary" />{t(
        localeStore.t.devToolbar.appPreferences,
        'App Preferences',
      )}
    </h3>
    {#if onclose}
  ...
  <div class="min-h-0 flex-1 overflow-y-auto flex flex-col gap-3 p-3">
    ...
    <div class="flex flex-col gap-1">
      <h4 class="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">
        {t(localeStore.t.devToolbar.sectionApp, 'App')}
      </h4>
    ...
    <div class="flex flex-col gap-1 border-t border-white/[0.06] pt-2">
      <h4 class="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">
        {t(localeStore.t.devToolbar.sectionUser, 'User')}
      </h4>
      <div class="flex flex-col gap-3">
        ...
        <div class="flex gap-2">
          {#if isLoggedOut}
            <Button
              variant="secondary"
              size="sm"
              class="h-7 text-xs flex-1 bg-white/[0.10] hover:bg-white/[0.15] border-white/[0.08]"
              onclick={handleLogin}
            >
    ...
    <div class="flex flex-col gap-1 border-t border-white/[0.06] pt-2">
      <h4 class="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">
        {t(localeStore.t.devToolbar.sectionScenes, 'Scenes')}
      </h4>
    ...
    <div class="border-t border-white/[0.06] pt-2">
      <Button
        variant="secondary"
        size="sm"
        class="h-7 text-xs w-full bg-white/[0.10] hover:bg-white/[0.15] border-white/[0.08] {resetState ===
        'success'
          ? 'text-green-500'
          : ''}"
      ...
      <Popover.Root bind:open={openPicklists[pref.key]}>
        ...
        >
          <Command.Root>
            <div class="relative">
              <Command.Input
                bind:value={searchValues[pref.key]}
  ...
  {:else if pref.type === 'number'}
    <div class="flex items-center justify-between gap-3">
      ...
      <Input
        ...
        oninput={(e: Event) =>
          callSetter(pref.key, Number((e.target as HTMLInputElement).value) || 0)}
      />
    </div>
  {:else}
    <div class="flex items-center justify-between gap-3">
