...
<div class="flex flex-1 min-h-0 flex-col overflow-hidden" data-testid="dev-toolbar-flags">
  ...
  <div
    class="flex shrink-0 flex-col gap-3 border-b border-white/[0.06] bg-white/[0.06] px-3 py-2.5"
  >
    <div class="flex items-center justify-between">
      <h3 class="text-sm font-semibold text-foreground inline-flex items-center gap-2">
        <Flag class="size-4 text-primary" />{t(
          localeStore.t.devToolbar.featureFlags,
          'Feature Flags',
        )}
      </h3>
      {#if onclose}
    ...
    <div class="relative">
      ...
      <Input
        ...
        class="h-8 pl-8 {searchQuery
          ? 'pr-8'
          : 'pr-3'} text-xs md:text-xs bg-white/[0.06] border-white/[0.08]"
        value={searchQuery}
        oninput={(e: Event) => {
