...
**Status**: [x] — `tools/oxlint.ts` created with JSON transform, registered in `tools/registry.ts` (113 tools total), 9 tests added (1670 total). `package.json` updated: `qa:lint` now runs unified `cli.ts --tools --warn-only` (custom rules + all external tools including oxlint in one command). `qa:lint:custom` removed. QA clean.
